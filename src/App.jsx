...
const TABS = ["Dashboard", "Pantry", "Recipes", "Pricing", "Orders", "Schedule", "Social"];
...
// ─── INITIAL PANTRY ───────────────────────────────────────────────────────────
const INIT_PANTRY = [
  { id: 1,  name: "All-purpose flour",  category: "Flour & Grains", costPer: 0.22, unit: "cups",  storeUnit: "5lb bag",   storeCost: 3.99,  yields: 18  },
  { id: 2,  name: "Butter",             category: "Dairy",          costPer: 0.50, unit: "cups",  storeUnit: "4-stick",   storeCost: 5.49,  yields: 4   },
  { id: 3,  name: "Eggs",              category: "Eggs & Fats",    costPer: 0.42, unit: "pcs",   storeUnit: "dozen",     storeCost: 4.99,  yields: 12  },
  { id: 4,  name: "Brown sugar",        category: "Sweeteners",     costPer: 0.28, unit: "cups",  storeUnit: "2lb bag",   storeCost: 2.49,  yields: 5   },
  { id: 5,  name: "White sugar",        category: "Sweeteners",     costPer: 0.22, unit: "cups",  storeUnit: "5lb bag",   storeCost: 3.29,  yields: 11  },
  { id: 6,  name: "Vanilla extract",    category: "Flavoring",      costPer: 0.33, unit: "tsp",   storeUnit: "4oz bottle",storeCost: 7.99,  yields: 24  },
  { id: 7,  name: "Baking soda",        category: "Leavening",      costPer: 0.04, unit: "tsp",   storeUnit: "1lb box",   storeCost: 1.29,  yields: 96  },
  { id: 8,  name: "Salt",              category: "Leavening",      costPer: 0.01, unit: "tsp",   storeUnit: "26oz",      storeCost: 1.49,  yields: 149 },
  { id: 9,  name: "Chocolate chips",    category: "Chocolate",      costPer: 2.00, unit: "cups",  storeUnit: "12oz bag",  storeCost: 3.99,  yields: 2   },
  { id: 10, name: "Cream cheese",       category: "Dairy",          costPer: 0.31, unit: "oz",    storeUnit: "8oz block", storeCost: 2.49,  yields: 8   },
  { id: 11, name: "Heavy cream",        category: "Dairy",          costPer: 1.50, unit: "cups",  storeUnit: "pint",      storeCost: 2.99,  yields: 2   },
  { id: 12, name: "Powdered sugar",     category: "Sweeteners",     costPer: 0.29, unit: "cups",  storeUnit: "2lb bag",   storeCost: 2.29,  yields: 8   },
  { id: 13, name: "Cake box (10\")",    category: "Packaging",      costPer: 1.20, unit: "pcs",   storeUnit: "10-pack",   storeCost: 11.99, yields: 10  },
  { id: 14, name: "Cookie box (dozen)", category: "Packaging",      costPer: 0.83, unit: "pcs",   storeUnit: "12-pack",   storeCost: 9.99,  yields: 12  },
];

const INIT_RECIPES = [{
  id: 1, name: "Classic Chocolate Chip Cookies", servings: 24, category: "Cookies", photo: null,
  ingredients: [
    { pantryId: 1, name: "All-purpose flour", amount: 2.25, unit: "cups" },
    { pantryId: 2, name: "Butter",            amount: 1,    unit: "cups" },
    { pantryId: 3, name: "Eggs",             amount: 2,    unit: "pcs"  },
    { pantryId: 4, name: "Brown sugar",       amount: 0.75, unit: "cups" },
    { pantryId: 5, name: "White sugar",       amount: 0.75, unit: "cups" },
    { pantryId: 6, name: "Vanilla extract",   amount: 1,    unit: "tsp"  },
    { pantryId: 7, name: "Baking soda",       amount: 1,    unit: "tsp"  },
    { pantryId: 8, name: "Salt",             amount: 1,    unit: "tsp"  },
    { pantryId: 9, name: "Chocolate chips",   amount: 2,    unit: "cups" },
  ],
  notes: "Bake at 375°F for 9–11 minutes until golden.",
}];

const INIT_ORDERS = [
  { id: 1, customer: "Sarah M.", item: "Dozen cookies", due: "2026-05-03", status: "In Progress", total: 24, notes: "No nuts please", phone: "555-1234" },
  { id: 2, customer: "Jenny R.", item: "Birthday cake",  due: "2026-05-07", status: "Pending",     total: 65, notes: "Pink roses, 'Happy 30th!'", phone: "555-5678" },
];

const INIT_SOCIAL = [
  { id: 1, platform: "Instagram", type: "Product Photo",     caption: "Fresh out of the oven! 🍪 Nothing beats a warm chocolate chip cookie. DM to order!", date: "2026-05-02", status: "Scheduled", photo: null },
  { id: 2, platform: "Facebook",  type: "Behind the Scenes", caption: "Saturday morning baking session in full swing! 🧁", date: "2026-05-04", status: "Draft", photo: null },
];
...
function generateTasksFromOrder(order) {
  ...
  const add = (daysBefore, task) => {
    ...
    return { id: `${order.id}-t${daysBefore}`, date: d.toISOString().split("T")[0], task, done: false, orderId: order.id, auto: true };
  };
  return [
  ...
  return (
    <div onClick={() => ref.current.click()} style={{
      ...
      {value
        ...
        : <div style={{ textAlign: "center", color: C.muted, fontSize: small ? 20 : 28 }}>
            📷{!small && <div style={{ fontSize: 12, marginTop: 4 }}>Add Photo</div>}
          </div>
      }
...
async function aiCaption(platform, type) {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "user", content: `You are a social media expert for a home bakery. Write an engaging ${platform} caption for a ${type} post. Warm, authentic, under 150 words, 1-2 emojis, a call to action, and 3-5 hashtags. Return ONLY the caption text, nothing else.` }]
    })
  });
...
async function aiScheduleSuggestions(orders) {
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "user", content: `Home bakery open orders:\n${list}\n\nSuggest 3 extra prep or business tasks (NOT standard bake/deliver tasks). Think: shopping runs, packaging supplies, customer follow-ups.\n\nReturn ONLY a JSON array: [{"task":"...","daysFromNow":1},...]. No markdown, no explanation.` }]
    })
  });
  const data = await res.json();
...
async function aiOrderConfirmation(order, bakeryName) {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      model: "claude-sonnet-4-20250514", max_tokens: 1000,
      messages: [{ role: "user", content: `You are a warm, professional home baker writing a customer order confirmation email.

Bakery name: ${bakeryName || "my home bakery"}
Customer name: ${order.customer}
Item ordered: ${order.item}
Due/pickup date: ${order.due}
Order total: $${order.total}
Special notes: ${order.notes || "none"}

Write a friendly, professional order confirmation email. Include:
- A warm greeting using the customer's first name
- A clear confirmation of what they ordered
- The pickup/delivery date
- The total amount and a note about when/how payment is expected
- Any special notes acknowledged warmly
- A thank-you closing with the bakery name

Keep it concise, warm, and professional. Return ONLY the email body text (no subject line), ready to copy and send.` }]
    })
  });
  const data = await res.json();
  return data.content?.[0]?.text || "";
...
export default function BakersHubPro() {
  ...
  const [social,      setSocial]      = useState(INIT_SOCIAL);
  const [schedule,    setSchedule]    = useState(() => {
    const auto = []; INIT_ORDERS.forEach(o => auto.push(...generateTasksFromOrder(o))); return auto;
  });
  ...
  const [emailModal,     setEmailModal]     = useState(null); // order being emailed
  ...
  const addPantryItem = () => {
    ...
    setPantry(p => [...p, { ...newPantry, id: Date.now(), costPer, storeCost: parseFloat(newPantry.storeCost), yields: parseFloat(newPantry.yields) }]);
    setNewPantry({ name: "", category: "Flour & Grains", storeCost: "", yields: "", unit: "cups", storeUnit: "" });
    setShowNewPantry(false);
  };

  const updatePantryPrice = (id, sc, sy) => {
    setPantry(p => p.map(x => x.id === id ? { ...x, storeCost: parseFloat(sc), yields: parseFloat(sy), costPer: parseFloat(sc) / parseFloat(sy) } : x));
  };

  // ── Recipes ───────────────────────────────────────────────────────────────
  const addRecipe = () => {
    if (!newRec.name) return;
    setRecipes(r => [...r, { ...newRec, id: Date.now() }]);
  ...
  // ── Orders ────────────────────────────────────────────────────────────────
  const syncAutoTasks = useCallback((updatedOrders) => {
    setSchedule(prev => {
      const manual = prev.filter(t => !t.auto);
      const auto = [];
      updatedOrders.forEach(o => auto.push(...generateTasksFromOrder(o)));
      return [...manual, ...auto];
    });
  }, []);
  ...
  const addOrder = () => {
    ...
    const o = { ...newOrder, id: Date.now(), total: parseFloat(newOrder.total) || 0 };
    const updated = [...orders, o];
    setOrders(updated); syncAutoTasks(updated);
  ...
  const genEmail = async (order) => {
    ...
    try {
      ...
      setEmailBody(body);
    } catch { setEmailBody("Sorry, couldn't generate the email. Please try again."); }
  ...
  // ── Schedule ──────────────────────────────────────────────────────────────
  const toggleTask = id => setSchedule(prev => prev.map(t => t.id === id ? { ...t, done: !t.done } : t));
  ...
  const getAiTasks = async () => {
    ...
    try {
      ...
      setSchedule(prev => [...prev, ...suggestions.map((s, i) => {
        const d = new Date(today); d.setDate(d.getDate() + (s.daysFromNow || i));
        return { id: Date.now() + i, date: d.toISOString().split("T")[0], task: s.task, done: false, auto: false, aiSuggested: true };
      })]);
    } catch {}
  ...
  const addTask = () => {
    ...
    setSchedule(prev => [...prev, { ...newTask, id: Date.now(), done: false, auto: false }]);
  ...
  const addPost = () => {
    ...
    setSocial(prev => [...prev, { ...newPost, id: Date.now() }]);
  ...
  const genCaption = async () => {
    ...
    try { setNewPost(p => ({ ...p, caption: "" })); const cap = await aiCaption(newPost.platform, newPost.type); setNewPost(p => ({ ...p, caption: cap })); } catch {}
    setCaptionLoading(false);
  };

  // ── Dashboard metrics ─────────────────────────────────────────────────────
  const deliveredRev   = orders.filter(o => o.status === "Delivered").reduce((s, o) => s + o.total, 0);
  const pendingRev     = orders.filter(o => o.status !== "Delivered").reduce((s, o) => s + o.total, 0);
  ...
  return (
    <div style={{ fontFamily: "Georgia, serif", minHeight: "100vh", background: C.bg, color: C.text }}>
      ...
      <div style={{ background: `linear-gradient(135deg, ${C.dark} 0%, ${C.accent} 60%, #d4722a 100%)`, padding: "22px 20px 0", color: "#fff", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ fontSize: 24, fontWeight: "bold", marginTop: 2 }}>🧁 Baker's Hub Pro</div>
        ...
        {tab === "Pantry" && (
          <div>
            ...
            <div style={{ fontSize: 13, color: C.muted, marginBottom: 12, lineHeight: 1.5 }}>
              Your master cost list. Update prices here and every recipe & pricing calculation updates automatically.
            </div>

            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
              {["All", ...PANTRY_CATS].map(cat => (
                <button key={cat} onClick={() => setPantryFilter(cat)} style={{
                  padding: "4px 12px", borderRadius: 20, fontSize: 11, fontWeight: "600", cursor: "pointer",
                  border: `1px solid ${C.accent}`, background: pantryFilter === cat ? C.accent : "#fff",
                  color: pantryFilter === cat ? "#fff" : C.accent, fontFamily: "Georgia, serif",
                }}>{cat}</button>
              ))}
            </div>
            ...
            {showNewPantry && (
              <div style={s.card}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <select value={newPantry.category} onChange={e => setNewPantry(p => ({ ...p, category: e.target.value }))} style={{ ...s.input, flex: 1 }}>
                    {PANTRY_CATS.map(c => <option key={c}>{c}</option>)}
                  </select>
                </div>
                <div style={{ fontSize: 12, color: C.mid, margin: "12px 0 6px", fontWeight: "600" }}>HOW YOU BUY IT</div>
                <div style={{ display: "flex", gap: 8 }}>
                  <div style={{ flex: 1 }}>
                    <label style={s.label}>Store unit (e.g. "5lb bag")</label>
                    <input placeholder="5lb bag" value={newPantry.storeUnit} onChange={e => setNewPantry(p => ({ ...p, storeUnit: e.target.value }))} style={s.input} />
                  </div>
                  <div style={{ width: 100 }}>
                    <label style={s.label}>Store cost $</label>
                    <input type="number" step="0.01" placeholder="3.99" value={newPantry.storeCost} onChange={e => setNewPantry(p => ({ ...p, storeCost: e.target.value }))} style={s.input} />
                  </div>
                </div>
                <div style={{ fontSize: 12, color: C.mid, margin: "12px 0 6px", fontWeight: "600" }}>HOW YOU USE IT IN RECIPES</div>
                <div style={{ display: "flex", gap: 8 }}>
                  <div style={{ flex: 1 }}>
                    <label style={s.label}>Recipe unit</label>
                    <select value={newPantry.unit} onChange={e => setNewPantry(p => ({ ...p, unit: e.target.value }))} style={s.input}>
                      {UNITS.map(u => <option key={u}>{u}</option>)}
                    </select>
                  </div>
                  <div style={{ flex: 1 }}>
                    <label style={s.label}>How many {newPantry.unit}s per store unit?</label>
                    <input type="number" step="0.1" placeholder="18" value={newPantry.yields} onChange={e => setNewPantry(p => ({ ...p, yields: e.target.value }))} style={s.input} />
                  </div>
                </div>
                {newPantry.storeCost && newPantry.yields && (
                  <div style={{ background: C.light, borderRadius: 8, padding: "8px 12px", marginTop: 10, fontSize: 13, color: C.mid }}>
                    💡 Cost per {newPantry.unit}: <strong style={{ color: C.dark }}>${(parseFloat(newPantry.storeCost) / parseFloat(newPantry.yields)).toFixed(3)}</strong>
                  </div>
                )}
                    ...
                    <div key={item.id} style={s.card}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                        <div>
                          ...
                          <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>{item.storeUnit} · ${item.storeCost?.toFixed(2)} → {item.yields} {item.unit}s</div>
                        </div>
                        <div style={{ textAlign: "right" }}>
                          <div style={{ fontWeight: "bold", fontSize: 16, color: C.dark }}>${item.costPer.toFixed(3)}</div>
                      ...
                      {editingPantry === item.id ? (
                        <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
                          <div style={{ flex: 1 }}>
                            <label style={s.label}>New store cost $</label>
                            <input type="number" step="0.01" defaultValue={item.storeCost} id={`sc-${item.id}`} style={s.input} />
                          </div>
                          <div style={{ flex: 1 }}>
                            <label style={s.label}>Yields ({item.unit}s)</label>
                            <input type="number" step="0.1" defaultValue={item.yields} id={`sy-${item.id}`} style={s.input} />
                          </div>
                          <div style={{ display: "flex", flexDirection: "column", justifyContent: "flex-end", gap: 4 }}>
                            <button onClick={() => { updatePantryPrice(item.id, document.getElementById(`sc-${item.id}`).value, document.getElementById(`sy-${item.id}`).value); setEditingPantry(null); }} style={{ ...s.btn, padding: "7px 12px", fontSize: 12 }}>✓ Save</button>
                            <button onClick={() => setEditingPantry(null)} style={{ ...s.btnSec, padding: "6px 12px", fontSize: 12 }}>Cancel</button>
                          </div>
                        </div>
                      ) : (
            ...
            {showNewRec && (
              <div style={s.card}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  <select value={newRec.category} onChange={e => setNewRec(r => ({ ...r, category: e.target.value }))} style={{ ...s.input, flex: 1 }}>
                ...
                {newRec.ingredients.map((ing, i) => (
                  <div key={i} style={{ fontSize: 12, color: C.mid, padding: "2px 0" }}>• {ing.amount} {ing.unit} {ing.name}</div>
                ))}
                <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                  ...
                  <select value={recIngInput.unit} onChange={e => setRecIngInput(x => ({ ...x, unit: e.target.value }))} style={{ ...s.input, width: 80 }}>
                    {UNITS.map(u => <option key={u}>{u}</option>)}
                  </select>
                  <button onClick={addRecipeIng} style={{ ...s.btn, padding: "8px 12px" }}>+</button>
                </div>
                <textarea placeholder="Notes / baking instructions" value={newRec.notes} onChange={e => setNewRec(r => ({ ...r, notes: e.target.value }))} style={{ ...s.input, marginTop: 8, height: 70, resize: "vertical" }} />
              ...
              return (
                <div key={r.id} style={s.card}>
                  <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                    <PhotoUpload value={r.photo} onChange={v => setRecipes(prev => prev.map(x => x.id === r.id ? { ...x, photo: v } : x))} small />
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 12, color: C.mid, marginTop: 3 }}>
                        Ingredient cost: <strong style={{ color: C.dark }}>${totalCost.toFixed(2)}</strong>
                        <span style={{ color: C.muted }}> · ${(totalCost / r.servings).toFixed(3)}/serving</span>
                      </div>
                      <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                        <button onClick={() => { setSelRecipe(selRecipe?.id === r.id ? null : r); setScale(1); }} style={{ ...s.btnSec, padding: "5px 12px", fontSize: 12 }}>
                          {selRecipe?.id === r.id ? "Close" : "View / Scale"}
                        </button>
                        <button onClick={() => { setPricingRecId(String(r.id)); setPricingSvgs(r.servings); setTab("Pricing"); }} style={{ ...s.btn, padding: "5px 12px", fontSize: 12 }}>
                          → Price It
                        </button>
                      </div>
                    </div>
                  ...
                  {selRecipe?.id === r.id && (
                    <div style={{ marginTop: 14 }}>
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12, alignItems: "center" }}>
                        ...
                        {[0.5, 1, 1.5, 2, 3].map(f => (
                          <button key={f} onClick={() => setScale(f)} style={{ padding: "4px 12px", borderRadius: 20, border: `1px solid #d4a07a`, background: scale === f ? C.accent : "#fff", color: scale === f ? "#fff" : C.accent, cursor: "pointer", fontSize: 12, fontWeight: "600", fontFamily: "Georgia, serif" }}>{f}×</button>
                        ))}
                      </div>
                      <div style={{ background: C.light, borderRadius: 10, padding: 12 }}>
                        <div style={{ fontWeight: "600", fontSize: 12, marginBottom: 6, color: C.dark, letterSpacing: 0.5 }}>
                          INGREDIENTS — {Math.round(r.servings * scale)} servings · est. cost ${(totalCost * scale).toFixed(2)}
                        </div>
                        {r.ingredients.map((ing, i) => {
                          const unitCost = calcIngCost(ing, pantry);
                          return (
                            <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: `1px solid #f0dece`, fontSize: 13 }}>
                              <span>{ing.name}</span>
                              <div style={{ textAlign: "right" }}>
                                <span style={{ fontWeight: "600", color: C.dark }}>{Math.round(ing.amount * scale * 100) / 100} {ing.unit}</span>
                                {unitCost !== null && <span style={{ color: C.muted, fontSize: 11, marginLeft: 6 }}>(${(unitCost * scale).toFixed(2)})</span>}
                              </div>
                            </div>
                          );
                        })}
        ...
        {tab === "Pricing" && (
          <div>
            ...
            <div style={s.card}>
                ...
                return (
                  <div style={{ background: C.light, borderRadius: 10, padding: 12, marginTop: 10 }}>
                    <div style={{ marginBottom: 8 }}>
                      <label style={s.label}>Quantity / servings you're selling</label>
                      <input type="number" value={pricingSvgs} onChange={e => { setPricingSvgs(+e.target.value); setSellQty(+e.target.value); }} style={{ ...s.input, width: 120 }} />
                    </div>
                    <div style={{ fontSize: 13 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", borderBottom: `1px solid ${C.border}`, color: C.mid }}>
                        <span>Recipe base ({r.servings} servings)</span>
                        <span style={{ fontWeight: "600", color: C.text }}>${batchCost.toFixed(2)}</span>
                      </div>
                      {r.ingredients.map((ing, i) => {
                        const c = calcIngCost(ing, pantry);
                        return (
                          <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "2px 0 2px 10px", fontSize: 12, color: C.muted }}>
                            <span>{ing.amount * sf} {ing.unit} {ing.name}</span>
                            <span>{c !== null ? `$${(c * sf).toFixed(2)}` : "—"}</span>
                          </div>
                        );
                      })}
                      <div style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", fontWeight: "bold", color: C.dark, marginTop: 4 }}>
                        <span>Scaled ingredient cost ({pricingSvgs} servings)</span>
                        <span>${(batchCost * sf).toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                );
              ...
              <div style={{ fontSize: 12, color: C.muted, marginBottom: 8 }}>Packaging, custom decor, delivery, etc.</div>
              {extraCosts.map((ec, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 13, padding: "3px 0", color: C.mid }}>
                  <span>{ec.name}</span><span>${parseFloat(ec.cost).toFixed(2)}</span>
                </div>
              ))}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                {[["Labor hrs", laborHrs, setLaborHrs, 0.5], ["$/hr", laborRate, setLaborRate, 1], ["Overhead %", overhead, setOverhead, 1], ["Markup %", markup, setMarkup, 1], ["Qty to sell", sellQty, setSellQty, 1]].map(([label, val, setter, step]) => (
                  <div key={label}>
                    ...
                    <input type="number" step={step} value={val} onChange={e => setter(+e.target.value)} style={s.input} />
                  </div>
                ))}
              </div>
            ...
            {priceResult && (
              <div style={{ ...s.card, background: `linear-gradient(135deg, ${C.dark}, ${C.accent})`, color: "#fff" }}>
                ...
                {[
                  ["Ingredient Cost",         `$${priceResult.ingCost.toFixed(2)}`],
                  ["Labor Cost",              `$${priceResult.labor.toFixed(2)}`],
                  ["Subtotal",                `$${priceResult.sub.toFixed(2)}`],
                  [`+ ${overhead}% Overhead`, `$${priceResult.withOH.toFixed(2)}`],
                  [`+ ${markup}% Markup`,     `$${priceResult.final.toFixed(2)}`],
                ].map(([label, val]) => (
                  <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: "1px solid rgba(255,255,255,0.15)", fontSize: 13 }}>
                    <span style={{ opacity: 0.85 }}>{label}</span><span>{val}</span>
                  </div>
                ))}
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12, fontWeight: "bold", fontSize: 18 }}>
                  <span>Price Per Unit</span>
                  <span style={{ background: "rgba(255,255,255,0.2)", padding: "4px 14px", borderRadius: 20 }}>${priceResult.perUnit.toFixed(2)}</span>
                </div>
        ...
        {tab === "Orders" && (
          <div>
              ...
              <div style={{ position: "fixed", inset: 0, background: "rgba(44,26,14,0.55)", zIndex: 100, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
                <div style={{ background: C.card, borderRadius: "20px 20px 0 0", padding: 20, width: "100%", maxWidth: 720, maxHeight: "85vh", display: "flex", flexDirection: "column", boxShadow: "0 -8px 40px rgba(0,0,0,0.2)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                    <div>
                      <div style={{ fontWeight: "bold", fontSize: 16, color: C.dark }}>✉️ Order Confirmation</div>
                      <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>For {emailModal.customer} · {emailModal.item}</div>
                    </div>
                  ...
                  {emailLoading ? (
                    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, color: C.muted }}>
                      ...
                      <div style={{ fontSize: 14 }}>Writing your confirmation email...</div>
                    </div>
                  ) : (
                    <>
                      <div style={{ marginBottom: 8 }}>
                        <label style={s.label}>Subject line (copy manually)</label>
                        <div style={{ ...s.input, fontSize: 13, color: C.mid, background: C.light }}>
                          Order Confirmed! {emailModal.item} — {emailModal.due}
                        </div>
                      </div>
                      ...
                      <textarea
                        ...
                        style={{ ...s.input, flex: 1, resize: "none", fontSize: 13, lineHeight: 1.7, minHeight: 220 }}
                      />
                      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                        <button onClick={copyEmail} style={{ ...s.btn, flex: 1 }}>
            ...
            <div style={{ ...s.card, padding: "12px 14px", display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ fontSize: 12, color: C.muted, whiteSpace: "nowrap" }}>🏷 Bakery name:</span>
              <input value={bakeryName} onChange={e => setBakeryName(e.target.value)} placeholder="My Home Bakery" style={{ ...s.input, fontSize: 13, padding: "6px 10px" }} />
            </div>
            ...
            {showNewOrder && (
              <div style={s.card}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  ...
                  <select value={newOrder.status} onChange={e => setNewOrder(o => ({ ...o, status: e.target.value }))} style={{ ...s.input, width: 130 }}>
                    {STATUS_LIST.map(st => <option key={st}>{st}</option>)}
                  </select>
                </div>
                <textarea placeholder="Customer notes (allergies, special requests...)" value={newOrder.notes} onChange={e => setNewOrder(o => ({ ...o, notes: e.target.value }))} style={{ ...s.input, marginTop: 8, height: 60, resize: "vertical" }} />
                <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
              ...
              <div key={o.id} style={{ ...s.card, borderLeft: `4px solid ${STATUS_COLORS[o.status] || "#ccc"}` }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: C.muted, marginTop: 3 }}>Due: {o.due}{o.phone && ` · ${o.phone}`}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontWeight: "bold", fontSize: 18, color: C.dark }}>${o.total}</div>
                    <span style={s.tag(STATUS_COLORS[o.status])}>{o.status}</span>
                  </div>
                </div>
                ...
                <div style={{ display: "flex", gap: 6, marginTop: 10, flexWrap: "wrap", alignItems: "center" }}>
                  {STATUS_LIST.map(st => (
                    <button key={st} onClick={() => updateOrderStatus(o.id, st)} style={{
                      padding: "4px 10px", borderRadius: 20, border: `1px solid ${STATUS_COLORS[st]}`,
                      background: o.status === st ? STATUS_COLORS[st] : "#fff", color: o.status === st ? "#fff" : STATUS_COLORS[st],
                      cursor: "pointer", fontSize: 11, fontWeight: "600", fontFamily: "Georgia, serif",
                    }}>{st}</button>
                  ))}
                  <button onClick={() => genEmail(o)} style={{ ...s.btnSec, padding: "4px 12px", fontSize: 11, marginLeft: "auto" }}>
        ...
        {tab === "Schedule" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={getAiTasks} disabled={aiTaskLoading} style={{ ...s.btnSec, fontSize: 12, padding: "7px 12px" }}>
                  {aiTaskLoading ? "⏳..." : "✨ AI Suggest"}
                </button>
            ...
            <div style={{ ...s.card, background: C.light, marginBottom: 14 }}>
              <div style={{ display: "flex", gap: 10, marginBottom: 6, fontSize: 11, color: C.muted }}>
                <span style={{ padding: "1px 6px", borderRadius: 8, background: "#3b82f611", color: "#3b82f6", fontWeight: "700" }}>auto</span> from orders
                <span style={{ padding: "1px 6px", borderRadius: 8, background: "#8b5cf611", color: "#8b5cf6", fontWeight: "700", marginLeft: 8 }}>AI</span> AI-suggested
              </div>
            ...
            {showNewTask && (
              <div style={s.card}>
                ...
                <input placeholder="Task description" value={newTask.task} onChange={e => setNewTask(t => ({ ...t, task: e.target.value }))} style={{ ...s.input, marginTop: 8 }} />
                <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                  <button onClick={addTask} style={s.btn}>Add</button>
        ...
        {tab === "Social" && (
          <div>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 14, overflowX: "auto" }}>
              {PLATFORMS.map(p => {
                const count = social.filter(post => post.platform === p).length;
                return (
            ...
            {showNewPost && (
              <div style={s.card}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                  <select value={newPost.platform} onChange={e => setNewPost(p => ({ ...p, platform: e.target.value }))} style={{ ...s.input, flex: 1 }}>
                    {PLATFORMS.map(p => <option key={p}>{p}</option>)}
                  </select>
                  <select value={newPost.type} onChange={e => setNewPost(p => ({ ...p, type: e.target.value }))} style={{ ...s.input, flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  ...
                  <select value={newPost.status} onChange={e => setNewPost(p => ({ ...p, status: e.target.value }))} style={{ ...s.input, flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                  <button onClick={genCaption} disabled={captionLoading} style={{ ...s.btnSec, fontSize: 12 }}>
            ...
            {social.map(post => (
              <div key={post.id} style={s.card}>
                <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                  {post.photo
                    ? <img src={post.photo} alt="" style={{ width: 64, height: 64, borderRadius: 10, objectFit: "cover", flexShrink: 0 }} />
                    : <div style={{ width: 64, height: 64, borderRadius: 10, background: C.light, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 13, lineHeight: 1.5, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: expandedPost === post.id ? "unset" : 3, WebkitBoxOrient: "vertical" }}>
                      {post.caption}
                    </div>
                    {post.caption?.length > 100 && (
                      <button onClick={() => setExpandedPost(expandedPost === post.id ? null : post.id)} style={{ background: "none", border: "none", color: C.accent, fontSize: 12, cursor: "pointer", padding: 0, marginTop: 4, fontFamily: "Georgia, serif" }}>
                ...
                <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
                  {["Draft", "Scheduled", "Posted"].map(st => (
                    <button key={st} onClick={() => setSocial(prev => prev.map(p => p.id === post.id ? { ...p, status: st } : p))} style={{
                      padding: "4px 10px", borderRadius: 20, fontSize: 11, fontWeight: "600", cursor: "pointer", fontFamily: "Georgia, serif",
                      border: `1px solid ${st === "Posted" ? "#10b981" : st === "Scheduled" ? "#3b82f6" : "#9a7a65"}`,
                      background: post.status === st ? (st === "Posted" ? "#10b981" : st === "Scheduled" ? "#3b82f6" : "#9a7a65") : "#fff",
                      color: post.status === st ? "#fff" : (st === "Posted" ? "#10b981" : st === "Scheduled" ? "#3b82f6" : "#9a7a65"),
                    }}>{st}</button>
                  ))}
                </div>
              </div>
