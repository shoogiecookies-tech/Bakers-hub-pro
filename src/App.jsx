...
function generateTasksFromOrder(order) {
  ...
  confunction LoginScreen({ onLogin }) {
...
function AppInner({ session }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (profileData) { setBakeryName(profileData.bakery_name || "My Home Bakery"); setBakeryLogo(profileData.bakery_logo || null); }
      setDbLoading(false);
    };
    load();
  }, [uid]);
  ...
  const saveApiKey = (key) => {
    localStorage.setItem("baker_api_key", key); setApiKey(key);
  ...
  const addPantryItem = async () => {
    ...
    const { data } = await supabase.from("pantry").insert([{ user_id: uid, name: newPantry.name, category: newPantry.category, cost_per: costPer, unit: newPantry.unit, store_unit: newPantry.storeUnit, store_cost: parseFloat(newPantry.storeCost), yields: parseFloat(newPantry.yields) }]).select().single();
  ...
  const addRecipe = async () => {
    ...
    const { data } = await supabase.from("recipes").insert([{ user_id: uid, name: newRec.name, category: newRec.category, servings: newRec.servings, notes: newRec.notes, photo: newRec.photo, ingredients: newRec.ingredients }]).select().single();
  ...
  const addOrder = async () => {
    ...
    const { data } = await supabase.from("orders").insert([{ user_id: uid, customer: newOrder.customer, item: newOrder.item, due: newOrder.due || null, status: newOrder.status, total: parseFloat(newOrder.total) || 0, notes: newOrder.notes, phone: newOrder.phone }]).select().single();
    if (data) {
  ...
  const saveEditOrder = async () => {
    if (!editOrder) return;
    const updates = { customer: editOrder.customer, item: editOrder.item, due: editOrder.due || null, status: editOrder.status, total: parseFloat(editOrder.total) || 0, notes: editOrder.notes, phone: editOrder.phone };
  ...
  const genEmail = async (order) => {
    ...
    try { setEmailBody(await aiOrderConfirmation(order, bakeryName)); }
    catch (e) { setEmailBody(e.message === "NO_KEY" ? "⚠️ No API key set. Go to Settings to add your Anthropic API key." : "Error generating email. Please try again."); }
    setEmailLoading(false);
  };
  const copyEmail = () => { navigator.clipboard.writeText(emailBody).then(() => { setEmailCopied(true); setTimeout(() => setEmailCopied(false), 2000); }); };
  ...
  const getAiTasks = async () => {
    ...
    try {
      ...
      for (const s of suggestions) {
        const d = new Date(today); d.setDate(d.getDate() + (s.daysFromNow || 0));
    ...
    } catch (e) { alert(e.message === "NO_KEY" ? "No API key set. Go to Settings to add your Anthropic API key." : "Error getting suggestions."); }
    setAiTaskLoading(false);
  };

  const addPost = async () => {
    if (!newPost.platform) return;
    const { data } = await supabase.from("social_posts").insert([{ user_id: uid, platform: newPost.platform, type: newPost.type, caption: newPost.caption, date: newPost.date || null, status: newPost.status, photo: newPost.photo }]).select().single();
  ...
  const genCaption = async () => {
    ...
    try { setNewPost(p => ({ ...p, caption: "" })); const cap = await aiCaption(newPost.platform, newPost.type); setNewPost(p => ({ ...p, caption: cap })); }
    catch (e) { setNewPost(p => ({ ...p, caption: e.message === "NO_KEY" ? "⚠️ Go to Settings to add your Anthropic API key." : "Error. Please try again." })); }
    ...
    <div style={{ fontFamily: "Georgia, serif", minHeight: "100vh", background: C.bg, color: C.text }}>
      <div style={{ background: `linear-gradient(135deg, ${C.dark} 0%, ${C.accent} 60%, #d4722a 100%)`, padding: "22px 20px 0", color: "#fff", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {bakeryLogo ? <img src={bakeryLogo} alt="logo" style={{ width: 48, height: 48, borderRadius: 12, objectFit: "cover", border: "2px solid rgba(255,255,255,0.3)" }} /> : <div style={{ fontSize: 36 }}>🧁</div>}
          <div style={{ flex: 1 }}>
        ...
        <div style={{ display: "flex", overflowX: "auto", marginTop: 16, gap: 2 }}>
          {TABS.map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ padding: "10px 13px", border: "none", cursor: "pointer", fontSize: 12, fontWeight: tab === t ? "700" : "400", borderRadius: "10px 10px 0 0", whiteSpace: "nowrap", background: tab === t ? "rgba(255,255,255,0.2)" : "transparent", color: "#fff", borderBottom: tab === t ? "3px solid #fff" : "3px solid transparent", opacity: tab === t ? 1 : 0.72, fontFamily: "Georgia, serif" }}>{t}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: "18px 16px", maxWidth: 720, margin: "0 auto" }}>
        ...
        {tab === "Dashboard" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
              {[{ label: "Delivered Revenue", value: `$${deliveredRev.toFixed(2)}`, icon: "💰", color: "#10b981" }, { label: "Pending Revenue", value: `$${pendingRev.toFixed(2)}`, icon: "⏳", color: "#f59e0b" }, { label: "Open Orders", value: openOrders, icon: "📦", color: "#3b82f6" }, { label: "Scheduled Posts", value: scheduledPosts, icon: "📱", color: "#8b5cf6" }].map(k => (
                <div key={k.label} style={{ ...s.card, display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ fontSize: 24 }}>{k.icon}</div>
                  <div><div style={{ fontSize: 20, fontWeight: "bold", color: k.color }}>{k.value}</div><div style={{ fontSize: 11, color: C.muted }}>{k.label}</div></div>
                </div>
              ))}
            </div>
            <div style={{ ...s.card, background: `linear-gradient(135deg, ${C.dark}, ${C.accent})`, color: "#fff" }}>
              ...
              {[["Delivered Revenue", `$${deliveredRev.toFixed(2)}`], ["Pipeline (pending)", `$${pendingRev.toFixed(2)}`]].map(([l, v]) => (
                <div key={l} style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: "1px solid rgba(255,255,255,0.15)", fontSize: 13 }}><span style={{ opacity: 0.8 }}>{l}</span><span>{v}</span></div>
              ))}
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, fontWeight: "bold", fontSize: 16 }}>
                <span>Total Pipeline</span><span style={{ background: "rgba(255,255,255,0.2)", padding: "2px 14px", borderRadius: 20 }}>${(deliveredRev + pendingRev).toFixed(2)}</span>
              </div>
            </div>
            <div style={s.card}>
              <div style={{ fontWeight: "bold", marginBottom: 10, color: C.dark }}>📅 Today's Tasks ({todayTasks.length})</div>
              {todayTasks.length === 0 ? <div style={{ color: C.muted, fontSize: 13 }}>No tasks today — enjoy! 🎉</div>
                : todayTasks.map(t => (
            ...
            <div style={s.card}>
              ...
              {orders.filter(o => o.status !== "Delivered").slice(0, 4).map(o => (
                <div key={o.id} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: `1px solid ${C.border}`, fontSize: 13 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}><span style={{ color: C.muted }}>{o.due}</span><span style={s.tag(STATUS_COLORS[o.status])}>{o.status}</span></div>
                </div>
              ))}
        ...
        {tab === "Pantry" && (
          <div>
            ...
            <div style={{ fontSize: 13, color: C.muted, marginBottom: 12 }}>Master cost list — update prices here and everything recalculates automatically.</div>
            ...
            {showNewPantry && (
              <div style={s.card}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  ...
                  <div style={{ flex: 1 }}><label style={s.label}>Yields</label><input type="number" step="0.1" value={newPantry.yields} onChange={e => setNewPantry(p => ({ ...p, yields: e.target.value }))} style={s.input} /></div>
                </div>
              ...
              {pricingRecId && (() => {
                ...
                return (
                  <div style={{ background: C.light, borderRadius: 10, padding: 12, marginTop: 10 }}>
                    <div style={{ marginBottom: 8 }}><label style={s.label}>Quantity selling</label><input type="number" value={pricingSvgs} onChange={e => { setPricingSvgs(+e.target.value); setSellQty(+e.target.value); }} style={{ ...s.input, width: 120 }} /></div>
          ...
          <div>
            ...
            <div style={s.card}>
              ...
              <div style={{ marginTop: 12 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  {bakeryLogo ? <img src={bakeryLogo} alt="logo" style={{ width: 72, height: 72, borderRadius: 14, objectFit: "cover", border: `2px solid ${C.border}` }} /> : <div style={{ width: 72, height: 72, borderRadius: 14, background: C.light, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, border: `2px dashed ${C.border}` }}>🧁</div>}
                  <div>
